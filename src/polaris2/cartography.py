...
def _plot_latlon_grid(ax, dr: Position, half: float):
    ...
    for lat_i in np.arange(lat0 - 10 * step, lat0 + 10 * step + 0.5 * step, step):
        ...
        if -half <= offy <= half:
            ...
            ax.text(half * 0.98, offy, f"{lat_i:.1f}°", fontsize=7, color="gray", ha="right", va="center")
    for lon_i in np.arange(lon0 - 10 * step, lon0 + 10 * step + 0.5 * step, step):
        ...
        if -half <= offx <= half:
            ...
            ax.text(offx, half * 0.98, f"{lon_i:.1f}°", fontsize=7, color="gray", ha="center", va="top")
...
def _plot_compass(ax, scenario: Scenario, half: float, colors):
    cx = half * 0.7
    cy = half * 0.7
    cr = half * 0.15
...
def plot_chart(scenario: Scenario) -> plt.Figure:
    dr = scenario.estimated_position
    half = max(scenario.dr_error_nmi * 4, 20.0)
    colors = plt.cm.Set1.colors
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.set_aspect("equal")
    ax.set_title("Navigation Chart (flat-plane)", fontsize=14, fontweight="bold")
    ax.set_xlabel("East — West (nmi)")
    ax.set_ylabel("North — South (nmi)")
    ...
    _plot_compass(ax, scenario, half, colors)
